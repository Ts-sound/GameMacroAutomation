...
class ScreenManager:
    ...
    def find_window(self, title: str) -> Optional[WindowInfo]:
        ...
        valid_windows = [
            w for w in windows if w.width >= min_width and w.height >= min_height
        ]

        if not valid_windows:
            # 如果没有符合条件的，返回最大的窗口
            valid_windows = sorted(
                windows, key=lambda w: w.width * w.height, reverse=True
            )
    ...
    def region_to_absolute(
        self, region: dict, screen_id: Optional[int] = None
    ) -> Tuple[int, int, int, int]:
        """
        将百分比区域转换为绝对坐标

        Args:
            region: 百分比区域 {"x": (0.0, 0.5), "y": (0.0, 1.0)}
            screen_id: 屏幕索引，None 使用主屏幕

        Returns:
            (abs_x, abs_y, abs_w, abs_h)
        """
        sid = screen_id if screen_id is not None else 0
        size = self.get_screen_by_id(sid)
        if size is None:
    ...
    def calculate_scale_factor(
        self, current_size: Tuple[int, int], reference_size: Tuple[int, int]
    ) -> float:
        """
        计算缩放因子
    ...
    def get_screen_region(
        self, window: Optional[WindowInfo], x: int, y: int, w: int, h: int
    ) -> Image.Image:
        """
        截取屏幕区域
    ...
    def auto_detect_scale_factor(
        self, window_title: str, reference_resolution: Tuple[int, int] = (1920, 1080)
    ) -> Optional[float]:
        """
        自动检测缩放因子
